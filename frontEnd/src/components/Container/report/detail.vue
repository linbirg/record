<template>
    <el-row class="dt-row">
        <el-col :span="22" class="">
            ...
            <el-checkbox v-model="input_item.completed" @change="postToUpdateItem"></el-checkbox>
	        <label class="line-1">{{index+1}}</label>
	        <el-input style="width:80%" v-if="is_editable" v-model="input_item.desc" @keyup.enter.native="enter_disable_input" placeholder="计划做的事情"></el-input>
	        <label v-else class="line-2" @dblclick="detail_text_click">{{input_item.desc}}</label>
        </el-col>

        <el-col :span="2">
            <span class="destroy" @click="detail_text_click"><i class="el-icon-edit"></i></span>
            <span class="destroy" @click="deleteCurrentItem" ><i class="el-icon-remove-outline"></i></span>
        </el-col>
    </el-row>
</template>
	...
	export default {
		...
		methods: {
            ...
            postToInsertItem(){
                ...
                this.post({
                    url: 'note/detail/add',
                    data: {'userId':userId, 'weekCount':this.weekCount, 'weekDay':this.weekDay, 'desc':this.input_item.desc, 'status':st}
                }).then(response => {
