...
async def chat_send_real(request):
    ...
    if check_summarization_needed(
        ...
        if check_summarization_debounce():
            asyncio.create_task(run_summarization(session_id, messages, user_id))
...
async def call_llm_stream(context: dict) -> dict:
    ...
    logger.LOG_INFO(
        f"[call_llm_stream] session_id={session_id}, messages_count={len(messages)}"
    )
...
请用中文回答用户的问题。""",
    ...
    logger.LOG_INFO(
        f"[call_llm_stream] model={conf.OPENAI_MODEL}, messages_to_send={len(openai_messages)}"
    )
    ...
    try:
        ...
        logger.LOG_INFO(
            f"[call_llm_stream] stream response received, type={type(response)}"
        )
        ...
        logger.LOG_INFO(
            f"[call_llm_stream] stream complete, content_length={len(full_content)}"
        )
...
async def chat_send(request):
    ...
    logger.LOG_INFO(
        f"[chat_send] user_id={user_id}, content={content[:50]}..., session_id={session_id}"
    )
    ...
    msg_store.save_user_message(content, {"user_id": user_id})
    logger.LOG_INFO(
        f"[chat_send] user message saved, message_count={msg_store.get_message_count()}"
    )
...
@post("/chat/history")
@ResponseBody
async def chat_history(userId, sessionId):
    """获取聊天历史"""
    ensure_sessions_initialized()
    ...
    if sessionId:
        ...
        messages = await ChatMessage.find_by_session(userId, sessionId)
        return {
            "messages": [
...
@post("/chat/clear")
@ResponseBody
async def chat_clear(userId, sessionId):
    """清空聊天历史"""
    await ChatMessage.delete_by_session(userId, sessionId)
