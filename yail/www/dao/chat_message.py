...
class ChatMessage(AutoIdModel):
    ...
    @classmethod
    async def find_by_session(cls, user_id, session_id, limit=50):
        sql = f"SELECT * FROM {cls.__table__} WHERE user_id=? AND session_id=? ORDER BY created_at ASC LIMIT ?"
        rows = await cls.select(sql, (user_id, session_id, limit))
        return [cls.row_mapper(r) for r in rows]

    @classmethod
    async def delete_by_session(cls, user_id, session_id):
        sql = f"DELETE FROM {cls.__table__} WHERE user_id=? AND session_id=?"
        await cls.execute(sql, (user_id, session_id))
